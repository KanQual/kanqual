...
function renderMissingAttributeValue() {
  return <span className="data-table-cell--muted">N/A</span>;
}
...
export function AIAssistAttributesView({
  projectId,
  ...
  async function saveAttributeValueForOwner(ownerId: string, value: string, action?: string, row?: AttributeSuggestionRow) {
    const attributeValueChange = action && row ? buildAttributeValueChangeMetadata(action, row) : undefined;
      ...
      const object = objects.find((item) => item.id === ownerId);
        if (!object) throw new Error(t("aiAssist.attributes.errors.suggestionObjectNotFound"));
      ...
      const relationship = relationships.find((item) => item.id === ownerId);
        if (!relationship) throw new Error(t("aiAssist.attributes.errors.suggestionRelationshipNotFound"));
  ...
  return (
    <div className="view view-shell ai-attribute-view">
      <ViewHeader
        title={t("aiAssist.attributes.title")}
        help={{ label: t("aiAssist.attributes.about"), onClick: () => setHelpOpen(true) }}
      />

      {helpOpen ? (
        <HelpModal
          title={t("aiAssist.attributes.helpTitle")}
          onClose={() => setHelpOpen(false)}
          lines={[t("aiAssist.attributes.helpBody")]}
      ...
      {loading ? <p className="supporting-copy">{t("aiAssist.attributes.loadingAttributes")}</p> : null}

      <div className="workbench-layout ai-assisted-coding-layout ai-assisted-coding-analyze-layout">
        <div className="workbench-sidebar">
          <div className="workspace-panel">
            <div className="workspace-panel-header">
              <span className="workspace-panel-title">{t("aiAssist.attributes.title")}</span>
            </div>
            ...
            <div className="data-table-wrap ai-attribute-picker-table-wrap">
              <table className="data-table ai-attribute-picker-table">
                <thead>
                  <tr>
                    {([
                      ...
                      <th
                        key={column.key}
                        className={`data-table-header ${column.className}${attributeSortColumn === column.key ? " data-table-header--sorted" : ""}`}
                ...
                <tbody>
                  {sortedAttributeTableRows.length === 0 ? (
                    ...
                    <tr
                      key={row.id}
                      className="data-table-row ai-attribute-picker-row"
                    ...
                    >
                      ...
                      <td className="data-table-cell">{row.typeLabel}</td>
                      <td className="data-table-cell ai-attribute-picker-missing-col">
                        <strong>{row.missingCount}</strong>
                        <span className="data-table-cell--muted"> / {row.totalCount}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="workspace-panel" style={{ marginTop: 16 }}>
            <div className="workspace-panel-header">
              <span className="workspace-panel-title">{t("aiAssist.attributes.landing.savedSuggestions")}</span>
              <button
                type="button"
        ...
        <div className="workbench-main">
          <div className="workspace-panel workspace-panel--grow">
            <div className="workspace-panel-header">
              ...
              <div className="ai-attribute-save-actions">
                <input
            ...
            {!selectedAttribute ? (
              ...
              <div className="ai-attribute-table-wrap">
                ...
                <table className="data-table ai-attribute-table">
                  <thead>
                    <tr>
                      <th className="data-table-header ai-attribute-table-select-col" aria-label={t("common.include")} />
                      <th className="data-table-header ai-attribute-table-owner-col">
                      ...
                      <th className="data-table-header ai-attribute-table-type-col">{t("aiAssist.chat.type")}</th>
                      <th className="data-table-header ai-attribute-table-value-col">{t("aiAssist.attributes.labels.currentValue")}</th>
                      <th className="data-table-header ai-attribute-table-value-col">{t("aiAssist.attributes.labels.suggestedValue")}</th>
                      <th className="data-table-header ai-attribute-table-action-col">{t("aiAssist.attributes.labels.review")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {attributeOwnerRows.map((ownerRow) => {
                      ...
                      return (
                        <tr key={ownerRow.ownerId} className={selectedOwnerIdSet.has(ownerRow.ownerId) ? "" : "ai-attribute-table-row--unselected"}>
                          <td className="data-table-cell ai-attribute-table-select-cell">
                            <input
                              type="checkbox"
                          ...
                          <td className="data-table-cell data-table-cell--name ai-attribute-table-owner-cell">{ownerRow.ownerName}</td>
                          <td className="data-table-cell ai-attribute-table-type-cell">{ownerRow.typeLabel}</td>
                          <td className="data-table-cell ai-attribute-table-value-cell">{ownerRow.currentValue ? ownerRow.currentValue : renderMissingAttributeValue()}</td>
                          <td className="data-table-cell ai-attribute-table-value-cell">
                          ...
                          <td className="data-table-cell ai-attribute-table-action-cell">
                            {suggestionRow?.suggestedValue.trim() ? (
