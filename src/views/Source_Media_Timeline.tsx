...
import { buildMultiAnnotationBackground, getFloatingTooltipStyle, tooltipExcerpt } from "./Source_Coding_Shared";
...
export type SourceMediaTimelineHandle = {
  zoomIn: () => void;
  zoomOut: () => void;
  fitToWaveform: () => void;
...
export type SourceMediaTimelineZoomUiState = {
  canZoomIn: boolean;
  canZoomOut: boolean;
  canFit: boolean;
  zoomPercent: number;
...
type SourceMediaTimelineProps = {
  ...
  seekBounds?: MediaSeekBounds | null;
  ...
  onZoomUiStateChange?: (state: SourceMediaTimelineZoomUiState) => void;
};
...
function buildZoomUiState(currentZoomPxPerSec: number, fitZoomPxPerSec: number): SourceMediaTimelineZoomUiState {
...
export const SourceMediaTimeline = forwardRef<SourceMediaTimelineHandle, SourceMediaTimelineProps>(({
  mediaElement,
  waveformCache,
  annotations,
  ...
  const seekBoundsRef = useRef<MediaSeekBounds | null>(seekBounds);
  ...
  useEffect(() => {
    ...
    seekBoundsRef.current = seekBounds;
  ...
  }, [mediaAnnotationIds, mediaAnnotations, onAnnotationContextMenu, onCreateSelection, onPlayClip, onSelectAnnotation, onUpdateAnnotationRange, onZoomUiStateChange, seekBounds]);
  ...
  useEffect(() => {
    ...
    if (!regions) {
      waveSurfer.destroy();
      setTimelineError(t("sourceCoding.media.waveformRegionsPluginFailed"));
      return;
    }
    ...
    const unsubscribers = [
      ...
      waveSurfer.on("interaction", (newTime) => {
        const bounds = seekBoundsRef.current;
        if (!bounds) return;
        const startSeconds = bounds.startMs / 1000;
        const endSeconds = bounds.endMs / 1000;
        const boundedTime = Math.min(endSeconds, Math.max(startSeconds, newTime));
        if (Math.abs(boundedTime - newTime) > 0.001) {
              ...
              {annotationStripBars.bars.map(({ annotation, background, color, lane, startMs, endMs }) => {
                ...
                return (
                  <button
                    key={annotation.id}
                    type="button"
                    title={`${annotation.codeLabels.join(", ") || t("sourceCoding.media.annotationFallback")} (${formatMediaTime(startMs)} - ${formatMediaTime(endMs)})`}
      ...
      {tooltip ? (
        ...
        >
          <div className="annotation-hover-tooltip-section">
            ...
            <div className="supporting-copy" style={{ margin: "6px 0 0" }}>{tooltip.timeLabel}</div>
...
SourceMediaTimeline.displayName = "SourceMediaTimeline";
