...
export function useSourceTextSizePreference() {
...
export type SourceCodingViewProps = {
  projectId: string;
  row: SourceRow;
  codes: PostgresCode[];
...
export function SourceCodebookCard({
  codes,
  selectedCodeId,
  ...
  return (
    <SettingsModal title={title} onClose={onCancel} closeDisabled={saving} modalClassName="modal--wide assoc-doc-modal">
      <div className="app-settings-modal-body">
        <p className="supporting-copy" style={{ marginBottom: selection.quote ? 12 : 16 }}>
          ...
          <label className="form-label">
            ...
            >
              {codeOptions.length === 0 ? (
                <p className="supporting-copy" style={{ margin: 0 }}>{t("sourceCoding.common.createCodeBeforeAnnotating")}</p>
              ) : (
...
export function SourceCodingFiltersModal({
  codes,
  annotations,
  hiddenUserIds,
  hiddenCodeIds,
                ...
                {visibleCodes.map(({ code, depth }) => (
                  <li key={code.id} className="annotation-filter-item">
                    <label className="annotation-filter-label">
                      ...
                      <span className="filter-option-count">{codeCounts.get(code.id) ?? 0}</span>
                    </label>
                  </li>
                ))}
              </ul>
                ...
                {users.map((user) => (
                  <li key={user.id} className="annotation-filter-item">
                    <label className="annotation-filter-label">
                      ...
                      <span className="filter-option-count">{user.count}</span>
                    </label>
                  </li>
                ))}
              </ul>
...
export function SourceAnnotationPanel({
  annotations,
  ...
  return (
    <div className="workspace-panel workspace-panel--grow">
      <div className="workspace-panel-header">
        <span className="workspace-panel-title">{t("sourceCoding.common.annotationsTitle", { count: annotations.length })}</span>
      </div>
...
export function SourceAnnotationContextMenu({
  contextMenu,
  contextMenuRef,
  contextMenuStyle,
  onClose,
