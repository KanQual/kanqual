...
import { DeleteIcon, EditIcon, HelpIcon, PlusIcon } from "../components/AppIcons";
...
import type { SourceEditorPayload, SourceRow } from "./Sources_View";
...
import type {
  SidebarAiStatus,
  SidebarCollaborationStatus,
  SidebarNetworkMode,
} from "./Sidebar";
import type { MemoDraftTarget } from "./Project_Memos_View";
import {
  GraphConfirmModal,
  TypeScopedAttributeModal,
  type TypeScopedAttributeDraft,
} from "./Project_Home_Modals";
import {
  HomeAnnotationImagePreview,
  truncateHomeAnnotationPreview,
} from "./Project_Home_Graph_Inspector";
import {
...
} from "./Project_Home_Timeline_Fields";
import {
...
const AppSettingsViewLazy = lazy(
  () => import("./App_Settings_View").then((m) => ({ default: m.AppSettingsView })),
);
const UserSettingsViewLazy = lazy(
  () => import("./User_Settings_View").then((m) => ({ default: m.UserSettingsView })),
);
const ProjectSourcesViewLazy = lazy(
  () => import("./Project_Sources_View").then((m) => ({ default: m.ProjectSourcesView })),
);
const SourceImportModalLazy = lazy(
  () => import("./Sources_View").then((m) => ({ default: m.SourceImportModal })),
);
const SourceEditorModalLazy = lazy(
  () => import("./Sources_View").then((m) => ({ default: m.SourceEditorModal })),
);
const ProjectHomeDetailsViewLazy = lazy(
  () => import("./Project_Home_Details_View").then((m) => ({ default: m.ProjectHomeDetailsView })),
);
const ProjectHomeTimelineViewLazy = lazy(
  () => import("./Project_Home_Timeline_View").then((m) => ({ default: m.ProjectHomeTimelineView })),
);
const ProjectHomeGraphViewLazy = lazy(
  () => import("./Project_Home_Graph_View").then((m) => ({ default: m.ProjectHomeGraphView })),
);
const AnalysisCodeSourcesViewLazy = lazy(
  () => import("./Analysis_Code_Sources_View").then((m) => ({ default: m.AnalysisCodeSourcesView })),
);
function RemovedCanvasPlaceholder(_props: { children?: ReactNode; [key: string]: unknown }) {
  return null;
}

const AnalysisDrawViewLazy = RemovedCanvasPlaceholder;
const AnalysisDrawCanvasViewLazy = RemovedCanvasPlaceholder;
const AnalysisNetworkViewLazy = RemovedCanvasPlaceholder;
const AIAssistAssistedCodingViewLazy = lazy(
  () => import("./AIAssist_Assisted_Coding_View").then((m) => ({ default: m.AIAssistAssistedCodingView })),
);
const FreeDrawCanvasViewLazy = RemovedCanvasPlaceholder;
const ExploreCanvasViewLazy = lazy(
  () => import("./Explore_Canvas_View").then((m) => ({ default: m.ExploreCanvasView })),
);
const ProjectMemosViewLazy = lazy(
  () => import("./Project_Memos_View").then((m) => ({ default: m.ProjectMemosView })),
);
const ReportsViewLazy = lazy(
  () => import("./Reports_View").then((m) => ({ default: m.ReportsView })),
);
const AIAssistHomeViewLazy = lazy(
  () => import("./AIAssist_Home_View").then((m) => ({ default: m.AIAssistHomeView })),
);
const AIAssistChatViewLazy = lazy(
  () => import("./AIAssist_Chat_View").then((m) => ({ default: m.AIAssistChatView })),
);
const AIAssistAnalyzeViewLazy = lazy(
  () => import("./AIAssist_Analyze_View").then((m) => ({ default: m.AIAssistAnalyzeView })),
);
const AIAssistAttributesViewLazy = lazy(
  () => import("./AIAssist_Attributes_View").then((m) => ({ default: m.AIAssistAttributesView })),
);
const AIAssistProcessSourcesViewLazy = lazy(
  () => import("./AIAssist_ProcessSources_View").then((m) => ({ default: m.AIAssistProcessSourcesView })),
);
...
export type ProjectHomeViewProps = {
...
function AIAssistPortPlaceholderView({
  title,
  detail,
}: {
  title: string;
  ...
  return (
    <div className="view home-view ai-assist-home-view">
      ...
      <section className="content-card ai-assist-home-card">
...
type ProjectHomeTab = "details" | "graph" | "timeline";
...
export function ProjectHomeView({
  ...
  const [projectHomeTab, setProjectHomeTab] = useState<ProjectHomeTab>("details");
    ...
    installationSettings == null
      ...
      : installationSettings.aiAssistPolicy.mode === "enabled"
        ...
        : installationSettings.aiAssistPolicy.mode === "project"
          ...
          : false;
  const [sidebarNetworkMode, setSidebarNetworkMode] = useState<SidebarNetworkMode>("unknown");
  const [sidebarAiStatus, setSidebarAiStatus] = useState<SidebarAiStatus>("unavailable");
  ...
  const [postgresMemoDraftTarget, setMemoDraftTarget] = useState<MemoDraftTarget | null>(null);
  ...
  const sidebarCollaborationStatus: SidebarCollaborationStatus =
    !project
      ? "disabled"
  ...
  function handleShowProjectObjects() {
    setSelectedObjectDetailsId(null);
    setActiveScreen("objects");
  }

  function handleShowProjectRelationships() {
    setSelectedRelationshipDetailsId(null);
    setActiveScreen("relationships");
  }
  ...
  const renderAnalysisDrawCanvas = () => (
    <Suspense fallback={<ViewLoadingFallback />}>
      <AnalysisDrawCanvasViewLazy onBack={() => setActiveScreen("analysis-draw")}>
        <div className="project-home-canvas-column">
          <ProjectHomeGraphViewLazy
              ...
              if (selection.kind === "relationship") {
                if (selection.relationship.id.startsWith("annotation:")) {
                  ...
                  const codeLabel =
                    ...
                    "";
                  const previewText = truncateHomeAnnotationPreview(
                  ...
                  return {
                    ...
                    preview:
                      annotation?.imageRegion && source?.storagePath ? (
                        ...
                        <div className="graph-canvas-inspector-preview">
                          <p className="graph-canvas-inspector-text">{previewText}</p>
                        </div>
                      ) : null,
                    attributes: [],
                  };
            ...
            drawCanvasToolbar={(
              <div className="saved-canvas-toolbar">
                <button
                  type="button"
  ...
  return (
    <div className="app-shell">
      <Sidebar
        ...
        onShowProjectObjects={handleShowProjectObjects}
        onShowProjectRelationships={handleShowProjectRelationships}
      ...
      <main className="app-main">
        <div className="view">
          {activeScreen === "home" ? (
            <div className="view view-shell project-home-view" style={{ minHeight: "100%" }}>
              <ViewHeader
                title={project.name || t("projectCore.home.untitledProject")}
                help={{ label: t("projectCore.home.openHelp"), onClick: () => setProjectHelpModal("home") }}
              />
              <div
                ...
                <div className={guideDetailsTourActive ? "getting-started-spotlight-target" : undefined}>
                  <Suspense fallback={<ViewLoadingFallback />}>
                    <ProjectHomeDetailsViewLazy
                      ...
                      onShowObjects={handleShowProjectObjects}
                      onShowRelationships={handleShowProjectRelationships}
            ...
            <>
              <div className="view view-shell project-view-shell">
                <ViewHeader
                  title={t("projectCore.homeShell.usersTitle")}
                  help={{ label: t("projectCore.homeShell.openUsersHelp"), onClick: () => setProjectHelpModal("users") }}
                />

                <div className="view-content project-view-content" style={{ alignItems: "stretch" }}>
                  {usersError ? <p className="alert-error">{usersError}</p> : null}

                  <MasterDetailLayout className="project-users-grid">
                    <div
                        ...
                        <div>
                          <table className="data-table" style={{ tableLayout: "fixed" }}>
                            <thead>
                              <tr>
                                <th
                                  className={`data-table-header${userRoleSortCol === "role" ? " data-table-header--sorted" : ""}`}
                                  style={{ width: "76%" }}
                                ...
                                <th
                                  className={`data-table-header${userRoleSortCol === "count" ? " data-table-header--sorted" : ""}`}
                                  style={{ width: "24%" }}
                    ...
                    >
                      {usersLoading ? (
                        ...
                        <TableShell className="data-table-wrap--elevated" style={{ maxHeight: 520 }}>
                          <table className="data-table">
                            <thead>
                              <tr>
                                <th className="data-table-header" style={{ width: "28%" }}>{t("projectCore.homeShell.user")}</th>
                                <th className="data-table-header" style={{ width: "18%" }}>{t("projectCore.homeShell.role")}</th>
                                <th className="data-table-header" style={{ width: "27%" }}>{t("projectCore.entities.created")}</th>
                                <th className="data-table-header" style={{ width: "27%" }}>{t("projectCore.homeShell.lastActive")}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {filteredProjectUsers.map((user) => (
                                <tr
                                  key={user.id}
                                  className="data-table-row project-data-table-row"
                                >
                                  <td className="data-table-cell data-table-cell--name">
                                    <div className="project-users-name-cell">
                                      <span>{user.name}</span>
                                    </div>
                                  </td>
                                  ...
                                  <td className="data-table-cell data-table-cell--muted">{formatCurrentDateTime(user.createdAt)}</td>
            ...
            <Suspense fallback={<ViewLoadingFallback />}>
              <ProjectMemosViewLazy
                projectId={project.id}
                projectStoragePath={project.storagePath}
                canManageMemos={canManageMemos}
                draftTarget={postgresMemoDraftTarget}
                onDraftHandled={() => setMemoDraftTarget(null)}
              />
            </Suspense>
          ) : activeScreen === "reports" ? (
            <Suspense fallback={<ViewLoadingFallback />}>
              <ReportsViewLazy projectId={project.id} projectStoragePath={project.storagePath} />
            </Suspense>
          ) : activeScreen === "ai-assist" ? (
            <Suspense fallback={<ViewLoadingFallback />}>
              <AIAssistHomeViewLazy
                project={project}
                authSession={authSession}
            ...
            <Suspense fallback={<ViewLoadingFallback />}>
              <AIAssistAssistedCodingViewLazy
                projectId={project.id}
                currentUserId={authSession.user.id}
                canManageSources={canManageSources}
                canKickSourceLocks={canManageSources}
            ...
            <>
              <div className="view view-shell">
                <ViewHeader
                  title={t("projectCore.homeShell.objects.detailsTitle")}
                  back={{
                    label: t("projectCore.homeShell.objects.back"),
                  ...
                  actions={(
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <button
                        ...
                        title={t("projectCore.homeShell.objects.editObject")}
                        aria-label={t("projectCore.homeShell.objects.editObject")}
                      >
                      ...
                      <button
                        ...
                        aria-label={t("projectCore.homeShell.objects.deleteObject")}
                      >
                ...
                <div className="view doc-detail-view" style={{ padding: 0, flex: 1, minHeight: 0 }}>
                        ...
                        {selectedObjectRelationshipRows.length > 0 ? (
                          <div className="case-detail-attributes-table-wrap">
                            <table className="data-table" style={{ tableLayout: "fixed" }}>
                              <thead>
                                <tr>
                                  <th className="data-table-header" style={{ width: "42%" }}>{t("projectCore.homeShell.objects.otherObject")}</th>
                                  <th className="data-table-header" style={{ width: "24%" }}>{t("projectCore.homeShell.objects.objectType")}</th>
                                  <th className="data-table-header" style={{ width: "34%" }}>{t("projectCore.homeShell.objects.relationship")}</th>
                                </tr>
                              </thead>
                              <tbody>
                                {selectedObjectRelationshipRows.map((row) => (
                                  <tr key={row.id} className="data-table-row">
                                    ...
                                    <td className="data-table-cell data-table-cell--muted">{row.otherObjectType}</td>
                                    <td className="data-table-cell data-table-cell--muted">
                                      <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                                        <RelationshipTypeLinePreview
            ...
            <>
              <div className="view view-shell">
                <ViewHeader
                  title={t("projectCore.homeShell.objects.title")}
                  help={{ label: t("projectCore.homeShell.objects.openHelp"), onClick: () => setProjectHelpModal("objects") }}
                />
                      ...
                      <div>
                        <table className="data-table" style={{ tableLayout: "fixed" }}>
                          <thead>
                            <tr>
                              <th
                                className={`data-table-header${objectTypeSortCol === "objectType" ? " data-table-header--sorted" : ""}`}
                                style={{ width: "76%" }}
                              ...
                              <th
                                className={`data-table-header${objectTypeSortCol === "count" ? " data-table-header--sorted" : ""}`}
                                style={{ width: "24%" }}
                          ...
                          <tbody>
                            ...
                            >
                              ...
                              <td className="data-table-cell data-table-cell--muted">{customObjects.length}</td>
                            </tr>
                            {objectTypeSummaries.map((summary) => (
                              <tr
                  ...
                  >
                    ...
                    {graphLoading ? (
                      ...
                      <>
                        {selectedObjectTypeFilter === "all" ? (
                          ...
                          <div className="content-card table-card">
                            <CardHeader
                              title={t("projectCore.homeShell.objects.attributes")}
                              actions={<button
                                type="button"
                          ...
                          <div className="data-table-wrap case-attributes-table-wrap">
                            <table className="data-table case-attributes-table">
                              <thead>
                                <tr>
                                  <th
                                    className={`data-table-header case-attributes-case-col${objectAttributeSortCol === "name" ? " data-table-header--sorted" : ""}`}
                                  ...
                                  {objectAttributeDefinitionsForWorkspace.map((definition) => (
                                    <th
                                      key={definition.id}
                                      className={`data-table-header case-attributes-value-col case-attributes-value-col--editable${objectAttributeSortCol === definition.id ? " data-table-header--sorted" : ""}${hoveredObjectAttributeColumnId === definition.id ? " case-attributes-col--hovered" : ""}`}
                              ...
                              <tbody>
                                {sortedObjectAttributeRows.length === 0 ? (
                                  <tr>
                                    <td colSpan={Math.max(objectAttributeDefinitionsForWorkspace.length + 1, 1)} className="data-table-message">
                                    ...
                                    <tr key={row.id} className="case-attributes-row">
                                      ...
                                      {objectAttributeDefinitionsForWorkspace.map((definition) => {
                                        ...
                                        return (
                                          <td
                                            key={definition.id}
                                            className={`data-table-cell case-attributes-value-cell${cellActive ? " case-attributes-cell--active" : ""}${hoveredObjectAttributeColumnId === definition.id ? " case-attributes-col--hovered" : ""}`}
                                            role="button"
                                            tabIndex={0}
                                            title={t("projectCore.homeShell.objects.viewAttributeHistory")}
                                            onClick={openHistory}
                      ...
                      <div className="content-card table-card">
                        ...
                        />
                      <TableShell className="data-table-wrap--elevated">
                        <table className="data-table">
                          <thead>
                            <tr>
                              <th className="data-table-header" style={{ width: "42%" }}>{t("projectCore.homeShell.objects.titleColumn")}</th>
                              <th className="data-table-header" style={{ width: "30%" }}>{t("projectCore.homeShell.objects.typeColumn")}</th>
                              <th className="data-table-header" style={{ width: "28%" }}>{t("projectCore.homeShell.objects.updated")}</th>
                            </tr>
                          </thead>
                          <tbody>
                            {filteredObjects.length === 0 ? (
                              ...
                              filteredObjects.map((object) => {
                                return (
                                  ...
                                  >
                                    <td className="data-table-cell data-table-cell--name">{object.title}</td>
                                    <td className="data-table-cell data-table-cell--muted">{object.objectType}</td>
                                    <td className="data-table-cell data-table-cell--muted">
                  ...
                  (() => {
                    ...
                    return (
                      ...
                      >
                          ...
                          <p className="supporting-copy" style={{ marginTop: 10, marginBottom: 0 }}>
            ...
            (() => {
              ...
              return (
                <div className="view view-shell">
                  <ViewHeader
                    title={t("projectCore.homeShell.relationships.detailsTitle")}
                    back={{
                      label: t("projectCore.homeShell.objects.back"),
                    ...
                    actions={(
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <button
                          ...
                          title={t("projectCore.homeShell.relationships.editRelationship")}
                          aria-label={t("projectCore.homeShell.relationships.editRelationship")}
                        >
                          <EditIcon className="card-header-icon" />
                        </button>
                        <button
                          ...
                          title={t("projectCore.homeShell.relationships.deleteRelationship")}
                          aria-label={t("projectCore.homeShell.relationships.deleteRelationship")}
                        >
                          <DeleteIcon className="card-header-icon" />
                        </button>
                      </div>
            ...
            <>
              <div className="view view-shell">
                <ViewHeader
                  title={t("projectCore.homeShell.relationships.title")}
                  help={{
                    label: t("projectCore.homeShell.relationships.openHelp"),
                      ...
                      <div>
                        <table className="data-table" style={{ tableLayout: "fixed" }}>
                          <thead>
                            <tr>
                              <th
                                className={`data-table-header${relationshipTypeSortCol === "relationshipType" ? " data-table-header--sorted" : ""}`}
                                style={{ width: "76%" }}
                              ...
                              <th
                                className={`data-table-header${relationshipTypeSortCol === "count" ? " data-table-header--sorted" : ""}`}
                                style={{ width: "24%" }}
                          ...
                          <tbody>
                            ...
                            >
                              ...
                              <td className="data-table-cell data-table-cell--muted">{relationships.length}</td>
                            </tr>
                            {relationshipTypeSummaries.map((summary) => (
                              <tr
                    ...
                    {graphLoading ? (
                      ...
                      <>
                        {selectedRelationshipTypeFilter === "all" ? (
                          ...
                          <div className="content-card table-card">
                            <CardHeader
                              title={t("projectCore.homeShell.relationships.attributes")}
                              actions={<button
                                type="button"
                                className="btn btn--primary card-header-icon-button"
                          ...
                          <div className="data-table-wrap case-attributes-table-wrap">
                            <table className="data-table case-attributes-table">
                              <thead>
                                <tr>
                                  <th
                                    className={`data-table-header case-attributes-case-col${relationshipAttributeSortCol === "name" ? " data-table-header--sorted" : ""}`}
                                  ...
                                  {relationshipAttributeDefinitionsForWorkspace.map((definition) => (
                                    <th
                                      key={definition.id}
                                      className={`data-table-header case-attributes-value-col case-attributes-value-col--editable${relationshipAttributeSortCol === definition.id ? " data-table-header--sorted" : ""}${hoveredRelationshipAttributeColumnId === definition.id ? " case-attributes-col--hovered" : ""}`}
                              ...
                              <tbody>
                                {sortedRelationshipAttributeRows.length === 0 ? (
                                  <tr>
                                    <td colSpan={Math.max(relationshipAttributeDefinitionsForWorkspace.length + 1, 1)} className="data-table-message">
                                    ...
                                    <tr key={row.id} className="case-attributes-row">
                                      ...
                                      {relationshipAttributeDefinitionsForWorkspace.map((definition) => {
                                        ...
                                        return (
                                          <td
                                            key={definition.id}
                                            className={`data-table-cell case-attributes-value-cell${cellActive ? " case-attributes-cell--active" : ""}${hoveredRelationshipAttributeColumnId === definition.id ? " case-attributes-col--hovered" : ""}`}
                                            role="button"
                                            tabIndex={0}
                                            title={t("projectCore.homeShell.relationships.viewAttributeHistory")}
                                            onClick={openHistory}
                      ...
                      <TableShell className="data-table-wrap--elevated">
                        <table className="data-table">
                          <thead>
                            <tr>
                              <th className="data-table-header" style={{ width: "28%" }}>{t("projectCore.homeShell.relationships.typeColumn")}</th>
                              <th className="data-table-header" style={{ width: "28%" }}>{t("projectCore.homeShell.relationships.from")}</th>
                              <th className="data-table-header" style={{ width: "28%" }}>{t("projectCore.homeShell.relationships.to")}</th>
                              <th className="data-table-header" style={{ width: "16%" }}>{t("projectCore.homeShell.relationships.updated")}</th>
                            </tr>
                          </thead>
                          <tbody>
            ...
            <Suspense fallback={<ViewLoadingFallback />}>
              <FreeDrawCanvasViewLazy />
            </Suspense>
          ) : activeScreen === "explore" ? (
            <Suspense fallback={<ViewLoadingFallback />}>
              <ExploreCanvasViewLazy
                objectTypes={objectTypes}
                objects={objects}
                relationships={relationships}
                relationshipTypes={relationshipTypes}
            ...
            <div className="view view-shell">
              ...
              <div className="empty-state empty-state--full-width" style={{ minHeight: 420 }}>
            ...
            <div className="view view-shell">
              ...
              >
                  ...
                  >
                    ...
                    <div style={{ maxHeight: 560, overflowY: "auto" }}>
                      <table className="data-table" style={{ tableLayout: "fixed" }}>
                        <thead>
                          <tr>
                            <th className="data-table-header" style={{ width: "62%" }}>{t("projectCore.homeShell.savedCanvases.mode")}</th>
                            <th className="data-table-header" style={{ width: "38%" }}>{t("projectCore.homeShell.savedCanvases.saved")}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {[
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="data-table-cell data-table-cell--muted">{count}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                ...
                <section className="view-content" style={{ alignItems: "stretch", justifyContent: "flex-start", gap: 16 }}>
                  {graphError ? <p className="auth-error">{graphError}</p> : null}
                  {graphLoading ? (
                    <div className="empty-state empty-state--full-width">
                    ...
                    <div className="data-table-wrap data-table-wrap--elevated">
                      <table className="data-table">
                        <thead>
                          <tr>
                            <th className="data-table-header" style={{ width: "30%" }}>{t("projectCore.homeShell.savedCanvases.name")}</th>
                            <th className="data-table-header" style={{ width: "16%" }}>{t("projectCore.homeShell.savedCanvases.mode")}</th>
                            <th className="data-table-header" style={{ width: "18%" }}>{t("projectCore.entities.created")}</th>
                            <th className="data-table-header" style={{ width: "18%" }}>{t("projectCore.entities.updated")}</th>
                            <th className="data-table-header" style={{ width: "18%" }}>{t("projectCore.homeShell.savedCanvases.actions")}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {filteredSavedDrawings.map((drawing) => (
                            <tr key={drawing.id} className="data-table-row">
                              <td className="data-table-cell data-table-cell--name">{drawing.name}</td>
                              <td className="data-table-cell data-table-cell--muted">{formatCanvasKindLabel(drawing.canvasKind)}</td>
                              <td className="data-table-cell data-table-cell--muted">
          ...
          {projectHelpModal ? (
            <HelpModal
              ...
              lines={POSTGRES_PROJECT_HELP_COPY[projectHelpModal].lineKeys.map((lineKey) => t(lineKey))}
            />
          ) : null}
          {homeCanvasAppearanceDraft ? (
            <PostgresCanvasAppearanceModal
