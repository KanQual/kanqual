...
import { ArrowLeftRightIcon } from "../components/AppIcons";
...
export type AIAssistChatViewProps = {
...
export function AIAssistChatView({
  ...
  return (
    <div className="view ai-chat-view">
      <ViewHeader
        title={t("aiAssist.chat.pageTitle")}
        help={{
          label: t("aiAssist.chat.openHelp"),
      ...
      {helpOpen && (
        <HelpModal
          title={t("aiAssist.chat.help.title")}
      ...
      {contextPickerOpen && (
        <SettingsModal title={t("aiAssist.chat.addContextTitle")} onClose={() => setContextPickerOpen(false)} modalClassName="modal--wide">
          <div className="app-settings-modal-body">
            <p className="supporting-copy">{t("aiAssist.chat.addContextBody")}</p>
            <div className="form-label" style={{ marginBottom: 16 }}>
              ...
              <p className="supporting-copy" style={{ marginTop: 8, marginBottom: 0 }}>
                  ...
                  <div className="ai-chat-context-picker-panel">
                    ...
                    <div className="ai-chat-context-modal-list" role="list">
                      ...
                      {contextTab === "document" && filteredSources.length === 0 && (
                        <p className="supporting-copy">{t("aiAssist.chat.noMatchingDocuments")}</p>
                      )}
                      {contextTab === "object" && filteredObjects.length === 0 && (
                        <p className="supporting-copy">{t("aiAssist.chat.noMatchingCases")}</p>
                      )}
                      {contextTab === "relationship" && filteredRelationships.length === 0 && (
                        <p className="supporting-copy">{t("aiAssist.chat.noMatchingRelationships")}</p>
                      )}
                      {contextTab === "code" && filteredCodes.length === 0 && (
                        <p className="supporting-copy">{t("aiAssist.chat.noMatchingCodes")}</p>
                      )}
                      {contextTab === "annotation" && filteredAnnotations.length === 0 && (
                        <p className="supporting-copy">{t("aiAssist.chat.noMatchingAnnotations")}</p>
                      )}
                      {contextTab === "memo" && filteredMemos.length === 0 && (
                        <p className="supporting-copy">{t("aiAssist.chat.noMatchingMemos")}</p>
                      )}
                    </div>
                  </div>
                </div>
