...
export function truncateHomeAnnotationPreview(value: string, maxLength = 100): string {
  const normalized = value.replace(/\s+/g, " ").trim();
  if (normalized.length <= maxLength) return normalized;
  return `${normalized.slice(0, maxLength).trimEnd()}...`;
}

export function HomeAnnotationImagePreview({
  projectStoragePath,
  sourceStoragePath,
  sourceKind,
  imageRegion,
    ...
    <div className="graph-canvas-inspector-preview">
      <div className="annotation-excerpt annotation-excerpt--clip">
        ...
        {loadError ? (
          ...
          <p className="supporting-copy" style={{ margin: 0 }}>{t("projectCore.graph.loadingAnnotationPreview")}</p>
        )}
      </div>
    </div>
  );
