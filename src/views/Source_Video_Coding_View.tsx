...
import type { PendingSelection, SourceAnnotationRow } from "./Sources_View";
import {
  AnnotationEditorModal,
  type AnnotationContextMenuState,
  orderedCodesWithDepth,
  type SourceCodingViewProps,
  SourceAnnotationContextMenu,
...
import {
  ...
  formatMediaTime,
  SourceMediaTimeline,
  type SourceMediaTimelineHandle,
  type SourceMediaTimelineZoomUiState,
} from "./Source_Media_Timeline";
import { NewCodeModal, type CodeRow } from "../components/NewCodeModal";
import "./Source_Media_Coding_View.css";
...
export function SourceVideoCodingView({
  row,
  codes,
  annotations,
  codeOptions,
  ...
  const [zoomUiState, setZoomUiState] = useState<SourceMediaTimelineZoomUiState>({
    canZoomIn: false,
    canZoomOut: false,
    canFit: false,
    zoomPercent: 100,
  ...
  const mediaTimelineRef = useRef<SourceMediaTimelineHandle | null>(null);
  ...
  return (
    <div className="view doc-detail-view">
      <ViewHeader
        title={t("sourceCoding.media.codeVideo")}
        ...
        help={{ label: t("sourceCoding.media.openHelp", { kind: "video" }), onClick: () => setHelpOpen(true) }}
      />

      <div className="workbench-layout coding-workspace-layout" style={{ minHeight: 0 }}>
        <div className="workbench-sidebar" style={{ display: "flex", flexDirection: "column", gap: 16, minHeight: 0 }}>
          <div className="workspace-panel" style={{ flexShrink: 0 }}>
            <div className="workspace-panel-header">
              <span className="workspace-panel-title">{t("sourceCoding.common.codebook")}</span>
              <button
                type="button"
        ...
        <div className="workbench-main">
          <div className="workspace-panel workspace-panel--grow media-source-card">
            ...
            {previewLoading ? (
              <p className="supporting-copy" style={{ margin: 0 }}>{t("sourceCoding.media.loadingPreview", { kind: mediaKind })}</p>
            ) : previewError ? (
              ...
              <div style={{ display: "grid", gap: 16, minHeight: 0 }}>
                <div className="media-player-console-frame">
                  ...
                  <div className="media-player-waveform-panel media-player-waveform-panel--video">
                    ...
                    <div className="media-player-transport-waveform-row">
                      <div className="media-player-transport-waveform-square" aria-label={t("sourceCoding.media.videoTransportControls")}>
                        <button
                          type="button"
                          className="media-player-transport-square-play"
                        ...
                        <div className="media-player-transport-square-step-row">
                          <button
                            type="button"
                            className="media-player-transport-square-step"
                      ...
                      <div className="media-player-transport-waveform-side-rect" aria-label={t("sourceCoding.media.videoSecondaryControls")}>
                        <div
                          className={`media-player-transport-side-cluster media-player-zoom-cluster${zoomControlOpen ? " is-zoom-open" : ""}`}
                        ...
                        >
                          <button
                            type="button"
                            className="media-player-transport-side-control media-player-zoom-fit-button"
                        ...
                        <div
                          className={`media-player-transport-side-cluster media-player-speed-cluster${speedControlOpen ? " is-speed-open" : ""}`}
                        ...
                        >
                          <button
                            type="button"
                            className="media-player-transport-side-control media-player-speed-button"
                            onClick={() => {
                        ...
                        <div
                          className={`media-player-transport-side-cluster media-player-volume-cluster${volumeControlOpen ? " is-volume-open" : ""}`}
                        ...
                        >
                          <button
                            type="button"
                            className="media-player-transport-side-control"
                      ...
                      />
                      {!activeClipRange ? (
                        <div className="media-player-transport-waveform-actions" aria-label={t("sourceCoding.media.videoClipActions")}>
                          <button
                            type="button"
                            onClick={createSelectionFromCurrentTime}
                            disabled={!canEditAnnotations}
                            className="media-player-transport-action-button media-player-transport-action-button--primary"
                          ...
                          <button
                            type="button"
                            className="media-player-transport-action-button"
                  ...
                  />
                  {activeClipRange ? (
                    <div className="media-player-transport-action-card" aria-label={t("sourceCoding.media.videoClipActions")}>
                      <>
                        <div className="media-player-transport-clip-fields" aria-label={t("sourceCoding.media.activeClipDetails")}>
                          <div className="media-player-transport-clip-field media-player-transport-clip-code-row">
                            <span className="media-player-transport-clip-field-label">{t("sourceCoding.common.codes")}</span>
                            <div className="media-player-transport-clip-code-list">
                              {activeClipCodes.length > 0 ? (
                                activeClipCodes.map((code) => (
                                  <span
                                    key={code.id}
                                    className="annotation-code-badge media-player-transport-clip-code-badge"
                                    style={{ background: code.color }}
                                  >
                                    <span className="media-player-transport-clip-code-label">{code.label}</span>
                                    <button
                                      type="button"
                                      className="media-player-transport-clip-code-remove"
                                      onClick={(event) => {
                                        event.stopPropagation();
                                    ...
                                    >
                                      <CloseIcon className="media-player-transport-clip-code-remove-icon" />
                                    </button>
                                  </span>
                                ))
                              ) : (
                                <span className="media-player-transport-clip-muted">{t("sourceCoding.media.notCoded")}</span>
                              )}
                            </div>
                          </div>
                          <label className="media-player-transport-clip-field">
                            <span className="media-player-transport-clip-field-label">{t("sourceCoding.media.start")}</span>
                            <input
                              type="text"
                              inputMode="decimal"
                          ...
                          <label className="media-player-transport-clip-field">
                            <span className="media-player-transport-clip-field-label">{t("sourceCoding.media.end")}</span>
                            <input
                              type="text"
                              inputMode="decimal"
                        ...
                        <div className="media-player-transport-clip-save-stack" aria-label={t("sourceCoding.media.activeClipSaveControls")}>
                          <button
                            type="button"
                            className="media-player-transport-clip-action-button media-player-transport-clip-action-button--save"
                          ...
                          <button
                            type="button"
                            className="media-player-transport-clip-action-button media-player-transport-clip-action-button--cancel"
                            onClick={cancelActiveClipDraft}
                            disabled={saving}
                            aria-label={t("sourceCoding.media.cancelClipEdits")}
                            title={t("common.cancel")}
                          >
                            <CloseIcon className="media-player-transport-clip-action-close-icon" />
                          </button>
                        </div>
                      </>
                    </div>
      ...
      {deletingCodeRow ? (
        <ConfirmDialog
          title={t("sourceCoding.common.deleteCodeTitle")}
          warning={t("sourceCoding.common.deleteCodeWarning")}
          error={deleteCodeError}
          busy={deletingCode}
          confirmLabel={t("sourceCoding.common.deleteCodeTitle")}
          busyLabel={t("sourceCoding.common.deleting")}
          tone="danger"
          onClose={() => setDeletingCodeId(null)}
          onConfirm={() => void handleConfirmDeleteCode()}
        >
      ...
      {helpOpen ? (
        <HelpModal
          title={t("sourceCoding.media.helpTitleVideo")}
          onClose={() => setHelpOpen(false)}
          lines={[t("sourceCoding.media.helpLine1"), t("sourceCoding.media.helpLine2")]}
        />
      ) : null}
      ...
      {clipDeleteConfirmation && canEditAnnotations ? (
        <ConfirmDialog
          ...
          onConfirm={() => void confirmClipAnnotationDelete()}
          busy={saving}
