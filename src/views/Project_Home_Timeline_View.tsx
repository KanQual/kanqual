...
function ProjectHomeTimelineSelectorCard({
  ...
  const { t } = useI18n();
  return (
    <section className="content-card project-home-selector-card">
      <div
        className="workspace-panel-header project-home-selector-card-header"
        role="button"
        tabIndex={0}
      ...
      {!collapsed ? (
        <>
          ...
          <div className="data-table-wrap project-home-selector-table-wrap">
            <table className="data-table project-home-selector-table">
              <thead>
                <tr>
                  <th className="data-table-header" style={{ width: "72%" }}>{t("common.name")}</th>
                  <th className="data-table-header" style={{ width: "28%" }}>{t("projectCore.entities.count")}</th>
                </tr>
              </thead>
              <tbody>
                {rows.length === 0 ? (
                  <tr>
                    <td className="data-table-message" colSpan={2}>{emptyText}</td>
                  </tr>
                ) : rows.map((row) => (
                  <tr
                    key={row.id}
                    ...
                    onClick={row.onClick}
                  >
                    <td className="data-table-cell data-table-cell--name" style={{ paddingLeft: row.indent ? 28 : undefined }}>
                      {row.label}
                    </td>
                    <td className="data-table-cell data-table-cell--muted">{row.count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export function ProjectHomeTimelineView({
  sources,
  sourceTypeSettings,
  ...
  const renderTimelineControlCluster = (embedded = false) => (
    <div
      className={`${embedded ? "" : "graph-canvas-canvas-controls "}project-home-timeline-controls`}
    ...
    >
      <div className="project-home-timeline-filter-control" ref={timelineFilterControlRef}>
        ...
        >
          <FilterIcon className="graph-canvas-canvas-control-icon" />
        </button>
        {timelineFilterMenuOpen ? (
          <div className="project-home-filter-drawer project-home-timeline-filter-menu">
            <div className="home-primary-column project-home-primary-column project-home-selector-column project-home-selector-column--drawer">
      ...
      <div className="graph-canvas-zoom-control" ref={timelineZoomControlRef}>
        <button
          type="button"
        ...
        {timelineZoomMenuOpen ? (
          <div className="graph-canvas-zoom-menu">
            <label className="graph-canvas-zoom-slider-label" htmlFor="project-home-timeline-zoom-slider">
  ...
  return (
    <section className={`project-home-timeline-card project-home-timeline-card--font-${timelineAppearance.fontSize}`} style={timelineCardStyle}>
      <div className="project-home-timeline-canvas-wrap">
            ...
            {selectedTimelineEntry ? (
              <div className="graph-canvas-inspector-overlay project-home-timeline-inspector-overlay">
                <section className="content-card graph-canvas-inspector-card">
                  <h2>{selectedTimelineEntry.title}</h2>
                  <div className="graph-canvas-inspector-kicker">
                    {selectedTimelineEntry.kind.charAt(0).toUpperCase() + selectedTimelineEntry.kind.slice(1)}
                  </div>
                  <div className="graph-canvas-inspector-type-detail">
