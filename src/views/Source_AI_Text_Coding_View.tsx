...
import type { SourceAnnotationRow } from "./Sources_View";
import {
...
export function SourceAITextCodingView({
  projectId,
  row,
  codes,
  annotations,
  ...
  async function applyRelevantSegment(segment: PostgresRelevantSegment) {
    ...
    const range = resolveRelevantSegmentRange(row.content, segment);
    if (!range) {
      setRelevantSegmentsError(t("sourceCoding.text.segmentNotFound"));
      return;
    }
  ...
  async function searchRelevantSegments() {
    if (!selectedCode) {
      setRelevantSegmentsError(t("sourceCoding.text.selectOneCodeBeforeSearching"));
      return;
    }
  ...
  return (
    <div className="view doc-detail-view">
            ...
            <div style={{ marginTop: 12, marginBottom: 12 }}>
              {sourceLockConflict?.reason === "kicked" ? (
                <p className="supporting-copy" style={{ margin: 0 }}>
                  {t("sourceCoding.common.sourceLockRemoved", {
                    userName: sourceLockConflict.userName || t("sourceCoding.common.projectEditor"),
                  })}
                </p>
              ) : sourceLockConflict?.reason === "locked" ? (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
                  <p className="supporting-copy" style={{ margin: 0 }}>
                    {t("sourceCoding.common.sourceLockHeld", {
                      userName: sourceLockConflict.userName || t("sourceCoding.common.anotherUser"),
                    })}
                  </p>
                  {canKickSourceLocks ? (
                    <button
            ...
            <div className="doc-viewer-inline-section ai-segments-embedded">
              <div className="doc-inline-section-header">
                ...
                >
                  ...
                  <span className="doc-inline-disclosure-label">{t("sourceCoding.text.relevantSegments")}</span>
                  <span className="ai-segments-inline-badge">{t("sourceCoding.text.aiSuggested")}</span>
                </button>
              ...
              {relevantSegmentsOpen && (
                <div className="doc-viewer-inline-section-body" style={{ maxHeight: 260 }}>
                  <div className="ai-segments-summary">
                    {selectedCode ? (
                      <>
                        <span className="ai-segments-summary-label">{t("sourceCoding.text.selectedCode")}</span>
                        <span className="ai-segments-summary-value">{selectedCode.label}</span>
                      </>
                    ) : (
                      <span className="annotation-list-empty">{t("sourceCoding.text.selectOneCodeBeforeSearching")}</span>
                    )}
                  </div>
                  {relevantSegmentsSearching && (
                    <div className="ai-segments-search-state">
                      ...
                      <div className="ai-segments-runtime">{t("sourceCoding.text.runtimeLabel")} {formatRuntime(relevantSegmentsRuntimeMs)}</div>
                      <div className="ai-segments-search-copy">{t("sourceCoding.text.searchingRelevantSegments")}</div>
                    </div>
                  )}
                  ...
                  {relevantSegmentsModel && !relevantSegmentsSearching && (
                    <p className="ai-segments-model-note">{t("sourceCoding.text.latestSearchModelLabel")} <code>{relevantSegmentsModel}</code></p>
                  )}
                  <ul className="annotation-list">
                    {!relevantSegmentsSearching && relevantSegments.length === 0 && selectedCode && !relevantSegmentsError && (
                      <li className="annotation-list-empty">{t("sourceCoding.text.runSearchPrompt")}</li>
                    )}
      ...
      {helpOpen ? (
        <HelpModal
          title={t("sourceCoding.text.helpTitle")}
      ...
      {deletingCodeRow ? (
        <ConfirmDialog
          ...
          warning="This removes the code from the codebook and clears it from existing annotations."
